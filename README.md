...
`v0.2.0` · `GPL-3.0` · `单文件分发` · `断网与 file:// 可用` · `数据不出本机` · `Chrome/Edge 88+` · `Firefox 85+` · `Safari 14.1+`
...
三档产物**功能与界面完全一致**，只差内置 Python 预置包数量（下表的字节数为 `v0.2.0` 三档构建实取，括号内 MB 按十进制换算：1 MB = 1,000,000 B）：

| 档位 | 预置包 | 体积 | 适合谁 |
| --- | --- | --- | --- |
| **完整版** `full` | 151 个 wheel | 227,487,828 B（227.49 MB） | `pyarrow` / `duckdb` / `pyproj` 等专业件 |
| **正常版** `normal` | 99 个 wheel | 155,439,074 B（155.44 MB） | 办公 + 科研 + 机器学习 + 绘图 |
| **极简版** `minimal` | 33 个 wheel | 73,942,139 B（73.94 MB） | `numpy` `pandas` `matplotlib` 等基础件 |
...
| PDF | 文本抽取（带页分隔）或逐页渲染成图片（72–300 DPI）双模式；二进制不进本地存储 |
| 办公文档 | `doc(x)` / `ppt(x)` / `xls(x)` 六格式本机解析（xlsx → CSV、pptx 带 `----- 第 N 页 -----` 标记），零网络 |
...
> 左：附件卡片与 PDF 逐页预览；右：办公文档本机解析（xlsx → CSV + 图片）。
...
产物内嵌的第三方库：marked 12.0.2 · DOMPurify 3.1.6 · highlight.js 11.9.0 · KaTeX 0.16.11 · pdf.js 6.3.289 · Tesseract.js 7.0.0（eng + chi_sim）· pyodide 314.0.6 · mammoth / SheetJS CE / docstream / fflate · JupyterLite 0.8.3 · jedi 0.19.2。版本、许可与出处逐条见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
