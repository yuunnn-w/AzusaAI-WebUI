...
`v0.2.1` · `GPL-3.0` · `单文件分发` · `断网与 file:// 可用` · `数据不出本机` · `Chrome/Edge 88+` · `Firefox 85+` · `Safari 14.1+`
...
三档产物**功能与界面完全一致**，只差内置 Python 预置包数量（下表的字节数为 `v0.2.1` 三档构建实取，括号内 MB 按十进制换算：1 MB = 1,000,000 B）：

| 档位 | 预置包 | 体积 | 适合谁 |
| --- | --- | --- | --- |
| **完整版** `full` | 151 个 wheel | 228,036,277 B（228.04 MB） | `pyarrow` / `duckdb` / `pyproj` 等专业件 |
| **正常版** `normal` | 99 个 wheel | 155,987,523 B（155.99 MB） | 办公 + 科研 + 机器学习 + 绘图 |
| **极简版** `minimal` | 33 个 wheel | 74,490,588 B（74.49 MB） | `numpy` `pandas` `matplotlib` 等基础件 |
