...
let kitSrc = read(KIT);
/* 胶水与包装层同样做 URL 中和(自检/注释里不应留可发起请求的地址) */
let glueNeutral = 0;
...
const banner = '<!-- ===== 内联办公文件解析库(mammoth 1.12.3 / SheetJS CE 0.20.3 / '
  + '@jose.espana/docstream 0.1.3 / fflate 0.8.3)+ OfficeKit 包装层 —— '
...
if (openCount !== LIBS.length + 2) { bad('script 块数量异常: ' + openCount + ',期望 ' + (LIBS.length + 2)); }
...
/* 文本块往返一致(逐字节可逆) */
LIBS.concat([{ id: GLUE_ID, header: null }]).forEach((l) => {
  const text = (l.id === GLUE_ID) ? glueText : shrunk[l.id].text;
  const back = unescapeInline(escapeForInline(text));
  if (back !== text) { bad(l.id + ' 转义后无法逐字节还原'); }
});
