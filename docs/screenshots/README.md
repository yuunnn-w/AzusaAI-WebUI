...
- 模型：真实 llama.cpp 端点（`Qwen3.8-27B-UD-Q6_K_M.gguf`，26 万 token 上下文、多模态）——采集时会话指向本机反向代理（当时的默认 Base URL 是 `http://127.0.0.1:8080/v1`；**自 v0.2.3 起该默认值改为 `http://127.0.0.1/v1`**，这批图是 v0.2.1 期实拍），代理再指向真实端点，故画面不出现任何内网地址；聊天屏均为真实流式生成，统计行（tok / tok·s⁻¹ / 秒）为服务端与端到端实测。
