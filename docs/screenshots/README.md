...
本目录存放 README 引用的实机截图：**真机 Thorium 122（原生 CDP）** 打开三档产物、连真实模型服务采集（非示意图），主题为马卡龙配色。旧 9 张（2026-09-14/15 采集，Chrome 152 headless）已于 2026-09-18 全部替换，旧版留档在 `../../../shared/archive/screenshots-old-2026-09-18/`。
...
| `05-office-parse.png` | 办公解析：xlsx（2 表 + 1 图）→ 预览弹窗内 CSV 文本 + 抽出的图片 | 浅色 · 薄荷 | 1600×900 | README「知识与文件」 |
...
| `08-jupyter.png` | JupyterLite 以工作区为根：`demo.ipynb` 已执行出结果 + 左侧文件浏览器 + 底部同步状态行 | JupyterLab 默认浅色 | 1600×900 | README「代码执行」 |
